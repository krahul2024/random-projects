import React, { useState } from 'react';
import { Link } from 'react-router-dom'; //for using links instead of anchor tags
import profile_icon from '../images/profile_icon.png'; //this is for using the profile icon from here as this is just one time so we just import the image
import { Toaster } from 'react-hot-toast'; //this and formik are for displaying messages and stuff for more info search about it
import { useFormik } from 'formik';
import styles from '../styles/username.module.css';
import { validateRegister } from '../helper/validate'
import convertToBase64 from '../helper/convert'



const Password = () => {

    const [file, setFile] = useState(); //this is for using the file uploads and it is imported from react

    const formik = useFormik({
        initialValues: {
            email: '',
            username: '',
            password: ''
        },
        validate: validateRegister,
        validateOnBlur: false,
        validateOnChange: false,
        onSubmit: async values => {
            values = await Object.assign(values, { profile: file || '' }) //this is setting the path for the image named file
            console.log(values);
        }

    })

    //formik doesn't support file upload 
    const onUpload = async event => {
        const base64 = await convertToBase64(event.target.files[0]) //converting the file in base64
        setFile(base64);
    }

    return ( < >
        <div className="container mx-auto">
        {/*this is for displaying the error message in top center of the screen when submission happens*/}
        <Toaster position='top-center' reverseOrder={false}></Toaster>

              <div className="flex justify-center items-center h-screen">

              <div className={styles.glass} style={{width:"35%"}}>

                <div className="title flex flex-col items-center">
                    
                <h3 className="text-3xl font-bold text-emerald-700">Welcome To The Club</h3>
                <span className="py-4 text w-2/3 text-center text-gray-400">Register and experience amazing stuff.</span>
                </div>

                {/*to use formik error message display feature we have to mention this formik thing here or anywhere the form is present*/}
                <form action="" className="py-1 " onSubmit={formik.handleSubmit}> 

                    {/*image of user*/}
                    <div className="profile flex justify-center items-center">
                    <label htmlFor="profile">
                    {/*here if there is no file path for image then we will consider default profile_icon as the profile picture*/}
                        <img className={styles.profile_image} src={file || profile_icon} alt="profile_icon"/>
                    </label>
                    <input onChange={onUpload} type="file" id="profile" name="profile"/>
                    </div>

                    {/*this is for input field*/}
                    <div className="textbox justify-center mt-12">
                    {/*as we have to display errors related to this particular field so we are using this formik statement in the username input field*/}
                    <input {...formik.getFieldProps('email')} className={styles.text_box  , styles.register} type="text" placeholder="E-mail*" />
                    <input {...formik.getFieldProps('username')} className={styles.text_box  , styles.register} type="text" placeholder="Username*" />
                    <input {...formik.getFieldProps('password')} className={styles.text_box , styles.register} type="password" placeholder="Password*" />
                    <button className={styles.btn} type="submit">Register</button>
                    </div>

                    <div className="text-center justify-center py-4">
                         <span className="text-gray-500 ml-4 text-md">Registered Already? 
                         <Link to="/" className="text-orange-400 ml-4 text-md">Log In</Link></span>
                    </div>

                </form>


              </div>
                
              </div>
                
            






            </div>

        < />
    )


}


export default Password;